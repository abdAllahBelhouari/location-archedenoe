<?php
	...
	$backPhotoArticle = "assets/img/back_800x600.jpg";

	if ( isset($_GET["deletePhoto"]) && $_GET["deletePhoto"] ) {
		$readArticle = $article->readArticle($_GET["article"]);
		if ( $readArticle["result"] ) {
			checkCsrf();
			$deletePhoto = $article->deletePhoto($_GET['article'], $_GET["deletePhoto"], $readArticle["response"]['photo'.$_GET['deletePhoto'].'Article']);
			if ( $deletePhoto["result"] ) { 
				setFlash("Félicitations.",$deletePhoto["response"]);
			} else {
				setFlash("Désolé !",$deletePhoto["response"],"danger");
			}
			header("location:?route=articles&article=".$_GET['article']);
			die();
		} else {
		...
		if ( $error ) {
			...
			if (isset($_GET["article"]) && $_GET["article"]){
				$updateArticle = $article->updateArticle($_GET["article"],$_POST, $_FILES);
		...
		if ( $readArticle["result"] ) {
			...
			for ( $p=1; $p < 4; $p++ ) { 
				if ( !is_null($_POST["photo".$p."Article"]) ) {
					$_POST["urlPhoto"][$p]= file_exists($folder.$_POST["photo".$p."Article"]) ? $folder.$_POST["photo".$p."Article"] : $backPhotoArticle;
				}
			}
		} else {
			setFlash("Désolé !",$readArticle["response"],"danger");
	...
	for ( $p=1; $p < 4; $p++ ) { 
		$_POST["urlPhoto"][$p] = isset($_POST["urlPhoto"][$p])  ? $_POST["urlPhoto"][$p] : $backPhotoArticle;
	}
			...
			<div class="row py-4">
				...
				<form action="#" method="POST" enctype="multipart/form-data"
						...
						<div class="col-sm-12 col-md-6 col-lg-6 col-xl-5">
							<div class="form-group">
								...
								<select name="idCategorie" id="idCategorie"
									...
									<option <?= (isset($_POST["idCategorie"])  && $_POST["idCategorie"]==$categorie["idCategorie"]) ? "selected" : ""; ?>
								...
								<div class="form-error">
									<?= $error['idCategorie'] ?? ''; ?></div>
							</div>
						</div>
						<div class="col-sm-12 col-md-6 col-lg-6 col-xl-7">
							<div class="form-group">
								<label for="libelleArticle"
									class="form-label">Libellé</label>
								<input type="text" name="libelleArticle"
									class="form-control" id="libelleArticle"
									value="<?=$_POST['libelleArticle']??"";?>">
								<div class="form-error">
									<?= $error['libelleArticle'] ?? ''; ?></div>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<textarea name="descriptionArticle" id="descriptionArticle" rows="5"
							class="form-control"><?=$_POST['descriptionArticle']??"";?></textarea>
					...
					<div class="row">
						<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6">
							<div class="form-group">
								<label for="achatArticle" class="form-label">Prix
									d'achat</label>
								<input type="text" name="achatArticle"
									class="form-control text-center"
									onkeypress="return VerifCasse(event,'float')"
									maxlength="7" id="achatArticle"
									value="<?= isset($_POST['achatArticle']) ? (float)$_POST['achatArticle'] : "";?>">
								<div class="form-error">
									<?= $error['achatArticle'] ?? ''; ?></div>
							</div>
						</div>
						<div class="col-sm-12 col-md-6 col-lg-6 col-xl-6">
							<div class="form-group">
								<label for="cautionArticle"
									class="form-label">Caution</label>
								<input type="text" name="cautionArticle"
									class="form-control text-center"
									onkeypress="return VerifCasse(event,'float')"
									maxlength="7" id="cautionArticle"
									value="<?=isset($_POST['cautionArticle']) ? (float)$_POST['cautionArticle'] : "";?>">
								<div class="form-error">
									<?= $error['cautionArticle'] ?? ''; ?></div>
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
							<div class="form-group">
								<label for="tarifHeureArticle" class="form-label">Tarif
									Heure</label>
								<input type="text" name="tarifHeureArticle"
									class="form-control text-center"
									onkeypress="return VerifCasse(event,'float')"
									maxlength="7" id="tarifHeureArticle"
									value="<?=isset($_POST['tarifHeureArticle']) ? (float)$_POST['tarifHeureArticle'] : "";?>">
								<div class="form-error">
									<?= $error['tarifHeureArticle'] ?? ''; ?></div>
							</div>
						</div>
						<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
							<div class="form-group">
								<label for="tarifJourArticle" class="form-label">Tarif
									Jour</label>
								<input type="text" name="tarifJourArticle"
									class="form-control text-center"
									onkeypress="return VerifCasse(event,'float')"
									maxlength="7" id="tarifJourArticle"
									value="<?=isset($_POST['tarifJourArticle']) ? (float)$_POST['tarifJourArticle'] : "";?>">
								<div class="form-error">
									<?= $error['tarifJourArticle'] ?? ''; ?></div>
							</div>
						</div>
						<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
							<div class="form-group">
								<label for="tarifWeekArticle" class="form-label">Tarif
									W-end</label>
								<input type="text" name="tarifWeekArticle"
									class="form-control text-center"
									onkeypress="return VerifCasse(event,'float')"
									maxlength="7" id="tarifWeekArticle"
									value="<?=isset($_POST['tarifWeekArticle']) ? (float)$_POST['tarifWeekArticle'] : "";?>">
								<div class="form-error">
									<?= $error['tarifWeekArticle'] ?? ''; ?></div>
							</div>
						</div>
					</div>
					<div class="row">
						<?php for ( $p=1; $p <4 ; $p++ ) : ?>
						<div class="col-sm-12 col-md-4 col-lg-4 col-xl-4">
							<label for="photo<?= $p; ?>Article" class="form-label">Photo
								<?= $p; ?></label>
							<div class="border rounded text-center divPhotoArticle">
								<input type="hidden" name="urlPhoto[<?= $p; ?>]"
									value="<?= $_POST["urlPhoto"][$p]; ?>">
								<input type="file" accept="image/*" class="photoArticle"
									id="photo<?= $p; ?>Article"
									name="photo<?= $p; ?>Article"
									onchange="showImgLoading(event, 'imgPhoto<?= $p; ?>Article');">
								<img id="imgPhoto<?= $p; ?>Article"
									src="<?= $_POST["urlPhoto"][$p]; ?>"
									class='imgPhotoArticle'
									onclick="document.getElementById('photo<?= $p; ?>Article').click();">
							</div>
							<?php if ( $_POST["urlPhoto"][$p] != "assets/img/back_800x600.jpg" ): ?>
							<a href="#" onclick="sweetAlert('Vous confirmez ?',
															'La suppression de cette photo sera définitive !',
															'?route=articles&article=<?= $_GET['article']; ?>&deletePhoto=<?= $p; ?>&<?= csrf(); ?>',
															'warning')" onclick="Processing();" class="mybtn-xs text-center">Supprimer</a>
							<?php endif; ?>
							<div class="form-error">
								<?= $error['photo'.$p.'Article'] ?? ''; ?></div>
						</div>
						<?php endfor; ?>
					</div>
					<div class="text-end mt-3">
						<a href="?route=articles" onclick="Processing()"
